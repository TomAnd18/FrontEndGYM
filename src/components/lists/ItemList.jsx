        ...
        <div className="w-auto flex min-w-0 gap-x-4 items-center">
          <div
            className={`h-7 w-7 rounded-full  ${
              activeCustomer ? "bg-teal-600" : "bg-red-600"
            } flex justify-center items-center`}
          >
            <p
              className={`${
                activeCustomer ? "text-white" : "text-white"
              } font-bold text-sm`}
            >{`${index + 1}`}</p>
          </div>
          <div className="min-w-0 flex-auto">
            <div className="text-md font-semibold leading-3 flex items-center">
              {person.gender == "Hombre" ? (
                <span className="w-3.5 h-3.5 mr-1">
                  <svg
                    ...
                    viewBox="0 0 448 512"
                    fill={`${activeCustomer ? "#1e3050" : "#dc2626"}`}
                  >
              ...
              <div
                className={`flex ${
                  activeCustomer ? "text-gray-600" : "text-red-600 font-bold"
                }`}
              >
            ...
            <p
              className={`mt-1 truncate text-xs leading-3 ${
                activeCustomer ? "text-gray-500" : "text-red-500 font-bold"
              }`}
            >
              {person.subscription}
            </p>
